...
const envSchema = z.object({
  ...
  AI_PROVIDER: z.enum(["disabled", "groq"]).default("disabled"),
  GROQ_API_KEY: z.preprocess(emptyToUndefined, z.string().optional()),
  GROQ_MODEL: z.preprocess(emptyToUndefined, z.string().optional()),
  AI_REQUEST_TIMEOUT_MS: z.coerce.number().int().min(1_000).max(60_000).default(12_000),
  AI_MAX_OUTPUT_TOKENS: z.coerce.number().int().min(1).max(4_096).default(700),
...
export const env = {
  ...
  GROQ_API_KEY: parsed.GROQ_API_KEY?.trim() || undefined,
  GROQ_MODEL: parsed.GROQ_MODEL?.trim() || undefined,
